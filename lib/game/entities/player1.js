...
.defines(function() {
	...
	EntityPlayer1 = ig.Class.extend({
		...
		draw: function() {
			...
			ig.system.context.strokeRect(this.hitBox.x1,this.hitBox.y1, this.hitBox.x2, this.hitBox.y2);
			if (this.currentAnim) {
	...
	EntityPlayer1.checkPair = function(a, b) {
		...
		if (
			...
			a.collides + b.collides > EntityPlayer1.COLLIDES.ACTIVE
		) {
			EntityPlayer1.solveCollision(a, b);
		}
	};


	EntityPlayer1.solveCollision = function(a, b) {

		// If one entity is FIXED, or the other entity is LITE, the weak
		// (FIXED/NON-LITE) entity won't move in collision response
		var weak = null;
		if (
			a.collides == EntityPlayer1.COLLIDES.LITE ||
			b.collides == EntityPlayer1.COLLIDES.FIXED
		) {
			weak = a;
		}
		else if (
			b.collides == EntityPlayer1.COLLIDES.LITE ||
			a.collides == EntityPlayer1.COLLIDES.FIXED
		) {
			weak = b;
		}
		...
		if (
			...
			if (a.last.y < b.last.y) {
				EntityPlayer1.seperateOnYAxis(a, b, weak);
			}
			else {
				EntityPlayer1.seperateOnYAxis(b, a, weak);
			}
			a.collideWith(b, 'y');
			b.collideWith(a, 'y');
		}
		...
		// Horizontal collision
			...
			if (a.last.x < b.last.x) {
				EntityPlayer1.seperateOnXAxis(a, b, weak);
			}
			else {
				EntityPlayer1.seperateOnXAxis(b, a, weak);
			}
			a.collideWith(b, 'x');
			b.collideWith(a, 'x');
		}
	...
	// themselfs.
	EntityPlayer1.seperateOnXAxis = function(left, right, weak) {
	...
	EntityPlayer1.seperateOnYAxis = function(top, bottom, weak) {
